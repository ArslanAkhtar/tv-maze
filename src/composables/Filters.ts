...
const useFilteredShows = () => {
  ...
  const filterShowsOnParams = (showArray: Show[], filterGenres?: boolean) => {
    const filteredShows = filterShows([...selectedGenres.value], selectedRating.value, showArray)
    if (filterGenres) {
  ...
  const showsCB = async () => {
    ...
    if (!query.value.length) {
      ...
      const extractShows = data?.value?.map((item: { score: number; show: Show }) => item.show)
      temp = extractShows
    }
